...
class LDAClassifierSoftmax(BaseEstimator):
	def __init__(self, lda_vectorizer, num_classes, learning_rate=0.01, max_iter=500, batch_size=50, converged=1e-8):
	...
	def fit(self, count_matrix, y):
		...
		gamma = np.array(self.lda_vectorizer.transform(count_matrix)) + 1e-100
		...
		while done == False:
			...
			for j in range(batchs):
				...
				E_theta_mb = E_theta[batch_ids]
				y_mb = y[batch_ids]

				# Gradient descent
				vt = self.learning_rate * self.gradient(E_theta_mb, y_mb)
				...
				# Loss of minibatch
				loss = self.loss(E_theta_mb, y_mb)
				if i % 10 == 0:
					loss_history.append(loss)
		print(loss_history)
		return self, loss_history

	def gradient(self, E_theta, y):
		grad = (softmax_stable(self.class_topic.dot(E_theta.T)) -\
			convert_labels(y, self.num_classes)).dot(E_theta)
		return grad

	def loss(self, E_theta, y):
		D = len(E_theta)
		score = softmax_stable(self.class_topic.dot(E_theta.T)) + 1e-100# CxD 
		true_class_scores = score[y, range(len(y))]
	...
	def predict(self, count_matrix):
		...
		D = len(gamma)
		E_theta = gamma / np.sum(gamma, axis=1).reshape(D, 1) # expectation of document topics 
		score = np.dot(E_theta, self.class_topic.T) # DxK . KxC = DxC
		t0 = time()
