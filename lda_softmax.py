...
from lib.lda_classifier_softmax import LDAClassifierSoftmax
...
def tune_lda(training_data, val_data, V, alphas, sizes, params):
	count_vect = CountVectorizer(max_df=.7, ngram_range=(1, 1), max_features=V)
...
if __name__ == '__main__':
	...
	# # LDA tune result
	make_dir('result/lda_softmax/%d/' % num_topics)
	# with open('result/lda_softmax/%d/lda-tune' % num_topics, 'w') as f:
	# 	f.write(str(val_params))
	# 	f.write(str(avg_perplexities))
	# save_pickle((val_params, avg_perplexities), 'result/lda_softmax/%d/lda-tune.pkl' % num_topics)	
	
	# val_params, avg_perplexities = load_pickle('result/lda_softmax/%d/lda-tune.pkl' % num_topics)
	...
	estimator = Pipeline([
		('count', CountVectorizer(max_df=.7, ngram_range=(1, 1), max_features=V)),
		('clf', LDAClassifierSoftmax(lda_vect, num_classes))
	])

	make_dir('result/lda_softmax/%d' % num_topics)
	estimator.fit(train_data, train_target)
	save_pickle(estimator, 'result/lda_softmax/%d/estimator.pkl' % (num_topics))
	estimator = load_pickle('result/lda_softmax/%d/estimator.pkl' % (num_topics))
	...
	with open('result/lda_softmax/%d/top-words.txt' % num_topics, 'w') as f:
	...
	with open('result/lda_softmax/%d/report' % num_topics, 'w') as f:
		f.write('Estimator:\n')
	...
	pool.close()
	pool.join()
	save_pickle((percent, f1), 'result/lda_softmax/%d/learning' % num_topics)
	# Plot
	plt.xlabel('Part of train data')
	plt.ylabel('F1 score')
	plt.plot(percent, f1, c='r')
	plt.legend()
	plt.savefig('result/lda_softmax/%d/learning_curve.png' % num_topics)
	# plt.show()
